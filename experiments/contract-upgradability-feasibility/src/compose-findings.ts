...
const TEST_ORDER = ['U1', 'U2', 'U3', 'U4', 'U5', 'U6', 'U7', 'U8', 'U9', 'U10'];
const TEST_DESCRIPTIONS: Record<string, string> = {
  U1:  'deploy v1; capture authority pubkey',
  U2:  'rotate authority; new key accepted, old key rejected',
  U3:  'authority loss / recovery — stale key replay',
  U4:  'removeVerifierKey isolates one circuit',
  U5:  're-insert original verifier key restores circuit',
  U6:  'logic upgrade — insert v2 VK at same circuit slot',
  U7:  'add brand-new circuit slot',
  U8:  'evolve ledger schema (new field)',
  U9:  'in-flight call vs maintenance tx — disruption window',
  U10: 'indexer visibility of maintenance events',
};
...
function buildTable(evidence: Map<string, Evidence>): string {
  ...
  for (const id of TEST_ORDER) {
    ...
    if (!e) {
      lines.push(`| ${padId} | PENDING | —                    | ${TEST_DESCRIPTIONS[id].padEnd(44).slice(0, 44)} |`);
      continue;
    }
    const tx = e.txHash ? trimTx(e.txHash) : e.errorCode ?? '—';
    lines.push(`| ${padId} | ${e.verdict.padEnd(7)} | ${tx.padEnd(20)} | ${(e.note || TEST_DESCRIPTIONS[id]).slice(0, 44).padEnd(44)} |`);
  }
  return lines.join('\n');
}
