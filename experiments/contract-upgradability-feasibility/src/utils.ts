...
// v1 — the deployed contract. Eagerly imported because every test needs it.
export const zkConfigPathV1 = path.resolve(__dirname, '..', 'contracts', 'managed', 'upgradable_v1');
const v1ContractPath = path.join(zkConfigPathV1, 'contract', 'index.js');
export const UpgradableV1 = await import(pathToFileURL(v1ContractPath).href);

// v2 — the build-only artefact. Verifier keys for v2 circuits are used by
// U6 (logic upgrade), U7 (new circuit), and U8 (new ledger field). Lazy
// so tests that don't need it can run before v2 is compiled.
export const zkConfigPathV2 = path.resolve(__dirname, '..', 'contracts', 'managed', 'upgradable_v2');
export async function loadV2(): Promise<any> {
  const v2ContractPath = path.join(zkConfigPathV2, 'contract', 'index.js');
  return await import(pathToFileURL(v2ContractPath).href);
}

// Back-compat alias — common.ts (copied verbatim from custody) reads
// `zkConfigPath` for paths it doesn't actually use any more, but keeping
// the symbol avoids edits to common.ts.
export const zkConfigPath = zkConfigPathV1;

export const PRIVATE_STATE_ID = 'upgradable-state';
